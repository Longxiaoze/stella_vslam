...
// FW:
// plp: include Plane and Line
#include "stella_vslam/optimize/local_bundle_adjuster_extended_plane.h"
#include "stella_vslam/optimize/local_bundle_adjuster_extended_line.h"
...
class Planar_Mapping_module; // FW: initialized in system.h and connected to mapper
...
public:
    ...
    // plp: Set the planar mapping module
    ...
    //-----------------------------------------
    // FW: used for debug (not used for real-time visualization of Pangolin)
    ...
    int _frame_num = 0; // used for saving image for debugging
...
private:
    ...
    void triangulate_with_two_keyframes(data::keyframe* cur_keyfrm, data::keyframe* ngh_keyfrm);

    // FW:
    void triangulate_line_with_two_keyframes(data::keyframe* cur_keyfrm, data::keyframe* ngh_keyfrm);
    ...
    // FW:
    // plp: Fuse duplicated landmarks between current keyframe and covisibility keyframes
    void fuse_landmark_duplication_line(const std::unordered_set<data::keyframe*>& fuse_tgt_keyfrms);
    ...
    // FW: TODO: local BA with plane and point (not working now, runnable but system crashes after few second)
    // plp: TODO: local BA with plane and point (not working now, runnable but system crashes after few second)
    std::unique_ptr<optimize::local_bundle_adjuster_extended_plane> _local_bundle_adjuster_extended_plane = nullptr;

    // FW: local BA with point and line
    // plp: local BA with point and line
    std::unique_ptr<optimize::local_bundle_adjuster_extended_line> _local_bundle_adjuster_extended_line = nullptr;
