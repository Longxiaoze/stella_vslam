...
// FW: from a frame
// plp: constructor for building from a frame
keyframe::keyframe(const frame& frm, map_database* map_db, bow_database* bow_db)
    : // meta information
      ...
      // FW:
      // plp: for line segment extractor
...
// FW: from map loading
// plp: constructor for building from map loading
...
// plp: constructor for building from map loading: points + lines
keyframe::keyframe(const unsigned int id, const unsigned int src_frm_id, const double timestamp,
                    const Mat44_t& cam_pose_cw, camera::base* camera, const float depth_thr,
                    const unsigned int num_keypts, const std::vector<cv::KeyPoint>& keypts,
                    const std::vector<cv::KeyPoint>& undist_keypts, const eigen_alloc_vector<Vec3_t>& bearings,
...
nlohmann::json keyframe::to_json() const {
    ...
    // plp: extract landmark_points or landmark_line IDs
...
// FW:
// plp: erase landmark line with index
...
// FW:
// plp: get number of tracked landmarks line
...
// plp: get keylines in cell same from frame.cc
...
// plp: triangulate stereo for line
...
float keyframe::compute_median_depth(const bool abs) const {
    ...
    // the 3rd row of rotation matrix
    const Vec3_t rot_cw_z_row = cam_pose_cw.block<1, 3>(2, 0);
    // z from the translation matrix
...
void keyframe::prepare_for_erasing() {
    ...
    // plp: remove associations between keylines and 3D lines
...
// FW:
// plp: initialize_lsd_scale_information
