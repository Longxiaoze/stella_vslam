...
void landmark::update_normal_and_depth() {
    ...
    // FW: calculate the average viewing direction for this map point
    // plp: calculate the average viewing direction for this map point
    ...
    // which level of image pyramid the keypoint is extracted
    const auto scale_level = ref_keyfrm->undist_keypts_.at(observations.at(ref_keyfrm)).octave;
    // the scale factor = pow(1.2, scale_level) corresponding to the level of image pyramid
...
//           /________\   level:predicted   --> d = cam_to_lm_dist
//          /__________\                     dmax/d = 1.2^m
// Farther /____________\ level:0   --> dmax
//
//                               log(dmax/d)
// predicted_scale_level = ceil(------------)
//                                log(1.2)
...
// plp: get the number of observations
Plane* landmark::get_Owning_Plane() const {
    std::lock_guard<std::mutex> lock(mtx_observations_);
    return _mOwningPlane;
}

void landmark::set_Owning_Plane(Plane* owning_pl) {
    std::lock_guard<std::mutex> lock(mtx_observations_);
