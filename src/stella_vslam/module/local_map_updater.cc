...
// FW:
std::vector<data::Line*> local_map_updater::get_local_landmarks_line() const {
    return _local_lms_line;
}
...
// FW:
bool local_map_updater::acquire_local_map_line() {
    const auto local_lms_line_was_found = find_local_landmarks_line();
    return local_lms_line_was_found;
}
...
// FW:
bool local_map_updater::find_local_landmarks_line()
{
    _local_lms_line.clear();
    _local_lms_line.reserve(50 * local_keyfrms_.size());

    for (auto keyfrm : local_keyfrms_)
    {
        const auto lms_line = keyfrm->get_landmarks_line();

        for (auto lm_line : lms_line)
        {
            if (!lm_line)
            {
                continue;
            ...
            // avoid duplication
            if (lm_line->_identifier_in_local_map_update == frm_id_)
            {
                continue;
            }
            lm_line->_identifier_in_local_map_update = frm_id_;

            _local_lms_line.push_back(lm_line);
        }
    }
