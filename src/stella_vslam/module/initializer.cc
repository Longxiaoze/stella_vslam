...
void initializer::create_initializer(data::frame& curr_frm) {
    ...
    // build a initializer
    //  std::unique_ptr<initialize::base>
    initializer_.reset(nullptr);
...
bool initializer::try_initialize_for_monocular(data::frame& curr_frm) {
    ...
    // FW: true -> check orientation
    // plp: true -> check orientation
...
bool initializer::create_map_for_monocular(data::frame& curr_frm) {
    ...
    // plp: also pass the corresponding segmentation mask
    ...
    // plp: triangulate lines in monocular initialization
    ...
    // plp: detect plane after Global BA, after monocular initialization
    if (map_db_->_b_seg_or_not) {
...
void initializer::scale_map(data::keyframe* init_keyfrm, data::keyframe* curr_keyfrm, const double scale) {
    ...
    // plp: scaling the landmarks line
...
bool initializer::create_map_for_stereo(data::frame& curr_frm) {
    ...
    // plp: also pass the corresponding segmentation mask, depth image, and rgb image
    if (map_db_->_b_seg_or_not)
    {
    ...
    // plp: (RGB-D/Stereo) create initial landmarks for 3D lines
    if (map_db_->_b_use_line_tracking)
    {
    ...
    // plp: detect plane after RGB-D initialization
    if (map_db_->_b_seg_or_not)
    {
...
// FW:
// plp: triangulate line with two keyframes
