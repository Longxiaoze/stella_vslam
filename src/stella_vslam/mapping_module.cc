...
// plp: include two_view_triangulator_line
#include "stella_vslam/module/two_view_triangulator_line.h"
...
// FW:
// plp: include Plane planar_mapping_module
#include "stella_vslam/planar_mapping_module.h"

namespace stella_vslam {

// plp: mapping_module with line and plane
mapping_module::mapping_module(data::map_database* map_db, const bool is_monocular)
    ...
    // plp: pass the boolean variable to map_cleaner
...
void mapping_module::mapping_with_new_keyframe() {
    ...
    // plp:-> compute BOW of cur_keyfrm
    //     -> update graph (check if associate landmark is linked to this keyframe)
    //     -> for the lm which is observed in the keyframe but not linked, add observation and update normal&depth and calculate descriptor
    //     -> add this keyframe into the map_db
    // plp: same procedure for 3D line landmark
    store_new_keyframe();
    ...
    // plp: similarly, try to match 2D line segments cross keyframes and triangulate 3D Line
    ...
    // plp: similar procedure for 3D lines
    update_new_keyframe();
    ...
    // FW: TODO:
    // plp: extended local BA with point and plane todo
...
void mapping_module::store_new_keyframe() {
    ...
    // plp: update graph (check 3D lines)
    if (map_db_->_b_use_line_tracking) {
    ...
    // plp: TODO connections updated according to 3D points, for now without 3D line
...
void mapping_module::create_new_landmarks() {
    ...
    for (unsigned int i = 0; i < cur_covisibilities.size(); ++i) {
        ...
        // plp: matching and triangulation in parallel
        if (map_db_->_b_use_line_tracking)
     {
...
void mapping_module::triangulate_with_two_keyframes(data::keyframe* cur_keyfrm, data::keyframe* ngh_keyfrm) {
    ...
    // plp: initialize the two view triangulator -> pass the keyframe pointer to that object, with poses information
...
#pragma omp parallel for
    ...
    for (unsigned int i = 0; i < matches.size(); ++i) {
        ...
        // plp: triangulation + check positive depth + check reprojection error + check scale consistency
...
// FW:
// plp: triangulate_line_with_two_keyframes
void mapping_module::triangulate_line_with_two_keyframes(data::keyframe* cur_keyfrm, data::keyframe* ngh_keyfrm) {
    if (cur_keyfrm->_keylsd.size() == 0 || ngh_keyfrm->_keylsd.size() == 0)
 {
    ...
    // plp: match line features  may be a better way to match lines !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
void mapping_module::update_new_keyframe() {
    ...
    // std::unordered_set<data::keyframe*>
    ...
    // FW:
    // plp: fuse_landmark_duplication_line
    if (map_db_->_b_use_line_tracking) {
    ...
    // plp: update the geometries (3D lines)
    if (map_db_->_b_use_line_tracking)
 {
    ...
    // plp: TODO: now not with 3D line for updating graph
    cur_keyfrm_->graph_node_->update_connections();
}
