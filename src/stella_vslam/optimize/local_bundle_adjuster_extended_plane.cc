...
#include "stella_vslam/optimize/local_bundle_adjuster_extended_plane.h"

#include "stella_vslam/data/keyframe.h"
#include "stella_vslam/data/landmark.h"
#include "stella_vslam/data/map_database.h"
#include "stella_vslam/optimize/g2o/landmark_vertex_container.h"
#include "stella_vslam/optimize/g2o/se3/shot_vertex_container.h"
#include "stella_vslam/optimize/g2o/se3/reproj_edge_wrapper.h"
...
// FW: 3D Plane using spherical coordinate
...
#include "stella_vslam/optimize/g2o/extended/point2plane_distance_edge_wrapper.h"
...
namespace stella_vslam
{
    namespace optimize
    {
        ...
        local_bundle_adjuster_extended_plane::local_bundle_adjuster_extended_plane(data::map_database *map_db,
        ...
        void local_bundle_adjuster_extended_plane::optimize(data::keyframe *curr_keyfrm, bool *const force_stop_flag) const
        {
            ...
            // FW: Aggregate 3D planes
            std::unordered_map<unsigned int, data::Plane *> local_planes;
            ...
            for (auto local_keyfrm : local_keyfrms)
            {
                ...
                for (auto local_lm : landmarks)
                {
                    ...
                    // FW: Aggregate 3D planes
                    if (!local_lm->get_Owning_Plane())
                    {
                        continue;
                    }

                    auto plane = local_lm->get_Owning_Plane();
                    if (plane->is_valid() && !plane->need_refinement())
                    {
                        if (local_planes.count(plane->_id))
                        {
                            continue;
                        }

                        local_planes[plane->_id] = plane;

                        // save plane parameters
                        Vec3_t n = plane->get_normal();
                        double d = plane->get_offset();
                        Vec4_t plane_function{n(0), n(1), n(2), d};
                        local_planes_function[plane->_id] = plane_function;
                    }
                }
            }
            ...
            // FW: connect 3D plane and landmark
            std::vector<optimize::g2o::extended::point2plane_edge_wrapper> point2plane_edge_wraps;
            point2plane_edge_wraps.reserve(local_lms.size() * local_planes.size());
            for (auto &id_local_lm_pair : local_lms)
            {
                auto local_lm = id_local_lm_pair.second;

                // get landmark g2o vertex
                auto lm_vtx = lm_vtx_container.get_vertex(local_lm);

                // add point-plane edge
                if (!local_lm->get_Owning_Plane())
                {
                    continue;
                }
                ...
                // create error edge between
                Vec4_t plane_function = local_planes_function[plane->_id];
                auto point2plane_edge_wrap = optimize::g2o::extended::point2plane_edge_wrapper(local_lm, lm_vtx,
                                                                                               plane_function);

                // aggregate edges
                point2plane_edge_wraps.push_back(point2plane_edge_wrap);
                optimizer.addEdge(point2plane_edge_wrap.edge_);
            }

            spdlog::info("point2plane_edge_wraps: {}", point2plane_edge_wraps.size());
