...
void local_bundle_adjuster::optimize(data::keyframe* curr_keyfrm, bool* const force_stop_flag) const {
    ...
    for (auto& id_local_keyfrm_pair : local_keyfrms) {
        ...
        // local_keyfrm->id_ == 0 return false, this vertex will not remain constant
    ...
    for (auto& id_fixed_keyfrm_pair : fixed_keyfrms) {
        ...
        // true: this vertex will remain constant
    ...
    for (auto& id_local_lm_pair : local_lms) {
        ...
        // false -> vertex not constant, will be optimized
    ...
    // initialize the optimizer we built before in steps [1]-[4]
    optimizer.initializeOptimization();
    // run the optimizer, num_first_iter_ = 5
