...
void global_optimization_module::run() {
    ...
    while (true) {
        ...
        // plp: [2] LoopClosing::ComputeSim3() in the ORB-SLAM2, find the ONE keyframe for closing the loop, calculate the Sim3, and get 3D-2D matches
        ...
        // plp: [3] Correct map and camera poses using the Sim3 calculated from step [2],
        // + Essential graph optimization (correct map and camera poses again),
        // + global BA (correct map and camera poses again).
        correct_loop();
    }
