...
viewer::viewer(const std::shared_ptr<stella_vslam::config>& cfg, stella_vslam::system* system,
...
void viewer::draw_keyframes() {
    ...
    std::vector<stella_vslam::data::keyframe*> keyfrms;
...
void viewer::draw_landmarks() {
    ...
    std::vector<stella_vslam::data::landmark*> landmarks;
    std::set<stella_vslam::data::landmark*> local_landmarks;
...
void viewer::draw_landmarks_plane() {
    if (!*_menu_show_plane)
 {
    ...
    std::vector<stella_vslam::data::Plane*> planes;
...
// FW:
void viewer::draw_dense_pointcloud(stella_vslam::data::keyframe* kf) {
    ...
    auto camera = static_cast<stella_vslam::camera::perspective*>(camera_);
...
void viewer::draw_landmarks_line() {
    ...
    if (!*_menu_show_lines)
 {
    ...
    std::vector<stella_vslam::data::Line*> landmarks_line;
    ...
    for (const auto lm : landmarks_line)
 {
        ...
        stella_vslam::Vec6_t pos_w = lm->get_pos_in_world();
