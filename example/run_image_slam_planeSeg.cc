...
#include "util/image_planeSeg_util.h"
                   ...
                   const std::string &map_db_path)
{
    ...
    // build a SLAM system
    stella_vslam::system SLAM(cfg, vocab_file_path, true); // true means: we have segmentation mask -> _b_seg_or_not = true
    ...
    std::thread thread([&]()
                       {
                           for (unsigned int i = 0; i < frames.size(); ++i)
                           {
                               ...
                               if (!img.empty() && (i % frame_skip == 0))
                               {
                                   ...
                                   SLAM.feed_monocular_frame(img, seg_mask_img, frame.timestamp_, mask);
                               }
