                       ...
                       const unsigned int frame_skip, const bool no_sleep, const bool auto_term)
{
    ...
    // build a SLAM system
    stella_vslam::system SLAM(cfg, vocab_file_path, false, true);
                       ...
                       const unsigned int frame_skip, const bool no_sleep, const bool auto_term)
{
    ...
    const tum_rgbd_sequence sequence(image_dir_path);
    const auto frames = sequence.get_frames();
...
int main(int argc, char *argv[])
{
    ...
    {
        rgbd_localization(cfg, vocab_file_path->value(), img_dir_path->value(), mask_img_path->value(),
                          map_db_path->value(), mapping->is_set(),
                          frame_skip->value(), no_sleep->is_set(), auto_term->is_set());
    }
    else
